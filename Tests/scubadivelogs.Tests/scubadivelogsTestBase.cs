...
using scubadivelogs.Migrations.SeedData;
...
namespace scubadivelogs.Tests
{
    public abstract class scubadivelogsTestBase : AbpIntegratedTestBase<scubadivelogsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected scubadivelogsTestBase()
        {
        ...
        protected void UsingDbContext(Action<scubadivelogsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<scubadivelogsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<scubadivelogsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<scubadivelogsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<scubadivelogsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<scubadivelogsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<scubadivelogsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<scubadivelogsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<scubadivelogsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<scubadivelogsDbContext, Task<T>> func)
        {
            T result;
